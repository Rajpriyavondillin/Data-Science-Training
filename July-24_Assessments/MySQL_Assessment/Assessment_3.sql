...
create database PetClinicManagementDB;
use PetClinicManagementDB;
...
select p.name,p.type,v.issue from pets p join visits v using(pet_id);

-- 4. Show the total number of visits per pet.
select p.name,count(v.pet_id) as TotalVisits 
from pets p join visits v using(pet_id) group by p.name;

-- Aggregation
-- 5. Find the total revenue collected from all visits.
select SUM(fee) as TotalRevenue from visits;

-- 6. Show the average age of pets by type.
select p.type,AVG(p.age) as AVG_Age from pets p 
join visits v using(pet_id) group by p.type;
...
select p.name,count(v.visit_id) as TotalVisits from pets p 
join visits v using(pet_id) group by p.name 
having TotalVisits>1;

-- Subqueries
-- 9. Show the pet(s) who had the costliest visit.
select p.name,v.fee from pets p join visits v 
using(pet_id) where v.fee=(select MAX(fee) from visits);
